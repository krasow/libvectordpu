...
void internal_launch_ternary(VectorDescRef res, VectorDescRef mask,
  ...
  for (uint32_t i = 0; i < nr_of_dpus; i++) {
    args[i].kernel = static_cast<uint32_t>(kernel_id);
    args[i].ktype = static_cast<uint8_t>(KERNEL_PIPELINE); // Ternary is always pipeline for now
  ...
  throw std::runtime_error("Standalone ternary kernels not implemented. Use JIT.");
}
