...
void allocator::deallocate_upmem_vector(detail::VectorDescRef data) {
  ...
  if (!data->desc.empty()) {
      ...
      // Let's rely on is_synchronized_ and just check if allocation was likely broadcast
      // If we differ, is_synchronized_ should have been false already.
      // But let's be safe: unique vectors might have different ptrs?
      // Actually, allocate_upmem_vector creates uniform ptrs if bump pointer is sync.
      // But if we ever supported non-uniform, we must check.
      // Checking 1024 items is O(N), but fast memory access.
      // Let's just check if is_synchronized_.
      if (is_synchronized_) {
