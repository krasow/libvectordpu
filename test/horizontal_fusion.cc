...
test_error test_horizontal_fusion_sums() {
  ...
  std::cout << "  CPU sum A: " << cpu_sum_a << ", DPU sum A: " << dpu_sum_a << std::endl;
  std::cout << "  CPU sum B: " << cpu_sum_b << ", DPU sum B: " << dpu_sum_b << std::endl;
  ...
  return TEST_SUCCESS;
#else
  std::cout << "Skipping horizontal fusion test (PIPELINE or JIT disabled)" << std::endl;
  return TEST_SUCCESS;
#endif
}
...
test_error test_linear_regression_like_fusion() {
#if PIPELINE && JIT
  const uint32_t N = 4096;
  std::cout << "Testing linear-regression-like horizontal fusion..." << std::endl;
  ...
  auto grad_x = dx.pipeline_reduce({OP_PUSH_INPUT, OP_PUSH_OPERAND_0, OP_MUL, OP_SUM}, {de});
  auto grad_y = dy.pipeline_reduce({OP_PUSH_INPUT, OP_PUSH_OPERAND_0, OP_MUL, OP_SUM}, {de});

  long long dpu_grad_x = (long long)grad_x.get();
  long long dpu_grad_y = (long long)grad_y.get();

  std::cout << "  CPU Grad X: " << cpu_grad_x << ", DPU Grad X: " << dpu_grad_x << std::endl;
  std::cout << "  CPU Grad Y: " << cpu_grad_y << ", DPU Grad Y: " << dpu_grad_y << std::endl;
